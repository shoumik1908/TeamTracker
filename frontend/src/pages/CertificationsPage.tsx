    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-[#1c1926]/80 backdrop-blur-md rounded-2xl shadow-2xl w-full max-w-lg border border-white/5">
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-white/5">
          <button onClick={onClose} className="flex-1 px-4 py-2 text-sm font-medium border border-white/5 rounded-lg hover:bg-muted">Cancel</button>
    ...
    <div className="space-y-5">
      <div className="page-header">
        ...
        <div className="flex items-center gap-2">
          <Link to="/tracker"
            className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-xl border border-white/10 hover:bg-white/5 text-white/90 hover:text-white transition-colors transition-colors">
      ...
      <div className="bg-[#1c1926]/80 backdrop-blur-md rounded-xl border border-white/5 p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading && Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="bg-[#1c1926]/80 backdrop-blur-md rounded-xl border border-white/5 p-5 h-44 animate-pulse">
        ...
        {data?.data.map(cert => (
          <div key={cert.id} className="bg-[#1c1926]/80 backdrop-blur-md rounded-xl border border-white/5 p-5 hover-card group">
            <div className="flex items-start justify-between gap-2 mb-2">
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm leading-tight">{cert.name}</p>
                <p className="text-xs text-white/50 mt-0.5">{cert.provider}</p>
              </div>
              </div>

            {cert.description && <p className="text-xs text-white/50 line-clamp-2 mb-3">{cert.description}</p>}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1">
                <button onClick={() => setEditCert(cert)}
                  className="p-1.5 text-white/50 hover:text-foreground hover:bg-muted rounded-lg transition-colors">
                  <Pencil className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => setDeleteId(cert.id)}
                  className="p-1.5 text-white/50 hover:text-red-400 hover:bg-red-950/40 rounded-lg transition-colors">
        ...
        {!isLoading && data?.data.length === 0 && (
          <div className="col-span-3 text-center py-12 text-white/50 bg-[#1c1926]/80 backdrop-blur-md rounded-xl border border-white/5">
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-[#1c1926]/80 backdrop-blur-md rounded-2xl shadow-2xl p-6 max-w-sm w-full border border-white/5">
            ...
            <p className="text-sm text-white/50 mb-6">This will also remove all assignments for this certification.</p>
