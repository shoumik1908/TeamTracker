...
const TRACKS: Array<{ id: Track; name: string; description: string; logo: string; className: string }> = [
  ...
  { id: 'FDE', name: 'FDE', description: 'Fabric Data Engineering', logo: '/coe-fde.svg', className: 'border-primary/25 bg-primary/5 text-primary' },
];
...
export default function CoePage() {
  ...
  const [activeView, setActiveView] = useState<'learning' | 'sessions'>('learning');
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 pb-8">
    <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      ...
      <div className="flex gap-1 border-t border-border px-4 pt-3"><button onClick={() => setActiveView('learning')} className={cn('rounded-lg px-3 py-2 text-sm font-semibold', activeView === 'learning' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-secondary')}>Learning board</button><button onClick={() => setActiveView('sessions')} className={cn('rounded-lg px-3 py-2 text-sm font-semibold', activeView === 'sessions' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-secondary')}>Knowledge Sharing Sessions</button></div>
    ...
    </section> : <KnowledgeSharingSessions />}
