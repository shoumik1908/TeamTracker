...
| Catalog integrity | Two certifications cannot share a name and provider; merging pre-existing duplicates keeps every holder, removes only an assignment a member already held on the surviving row, and leaves unrelated certifications untouched. |
| File and link attribution | An uploader or author that does not name a real team member is refused; deleting a member keeps their files and links and clears the attribution rather than blocking the deletion; a system-generated file records no uploader. |
| Audit trail | Every activity-log entry records both the actor's name at the time and a reference to their account; the reference is left empty when the recorded name matches more than one account, rather than guessing; deleting an account clears the reference but keeps the entry and the name. |
| Schema constraints | A member cannot be assigned to the same opportunity twice; deleting a user who created a learning project, CoE ticket or session is refused rather than cascading; a stage-change log cannot reference an opportunity that does not exist; member status, action-item status and priority, blocker status, pre-sales track, stage-change source, enrolment type, recording type and transcript source are database enums that reject any other value, and the enum migration preserves every existing value rather than dropping the columns. |
| Certification de-duplication | Merging duplicate catalog entries moves every assignment to the surviving row and loses no holder; where one member holds several copies, the record carrying the most evidence survives (uploaded certificate, then COMPLETED, then furthest progress) and the rest are removed; a member holding two copies that both have to move does not break the migration; unrelated certifications are untouched. |
| System-generated files | Files the proposal and section generators upload record no uploader rather than a placeholder id, so the actor columns can carry a real foreign key. |
| Meeting record enum fields | A meeting record's `recordingType` and `transcriptSource` are validated at the route: an unknown value is a 400 naming the allowed values, while absent, blank and `none` all mean the column stays empty. A deliberate 4xx must keep its status rather than being re-emitted as a 500 by a route's own catch. |
| Enum-valued request fields | Every request-fed value that lands in an enum column is validated at the route, so a caller mistake is a 400 naming the allowed values rather than a 500 from the database: member `status`, the pre-sales `account` delete filter (which still matches case-insensitively), and the stage-change `source` on both the stage and progress endpoints; omitting `source` still falls back to its default. |
...
| Dashboard | Admin and member dashboards, loading/error/empty states, KPI and chart navigation; "Tasks due this week" counts every open task actually due in the next seven days and reports a real overdue figure, compared by calendar day so the count does not shift with the reader's timezone; a render error shows a recovery message, never a stack trace. |
...
| Certification tracker | Filter/search/expand rows; upload with OCR credential; manual credential fallback; verified/unverified tag; duplicate guard; delete file; edit-request submission; a certification with no expiry date can still be uploaded; typing in search issues one query, not one per keystroke; logging a new certification whose name and provider already exist reuses the catalog entry instead of adding a second one, including after the dialog has been closed and reopened. |
| Notifications | Unread/read state, delete action, admin View edit request, Accept applies changes, Reject discards changes, buttons disappear after review; a team member navigating from their dashboard to Notifications renders the list rather than blanking the app — the two views no longer share one query key with different response shapes, and the dashboard's "Recent notifications" card shows its rows; a render error in any page is contained to the page area instead of unmounting the whole shell. |
...
- Keyboard-only checks with coverage today: the members table sorts via its column headers and announces the sort through `aria-sort`; a member row opens with Enter; CoE dialogs — both the CoE page's own and the learning-project ones — expose `role="dialog"`, take focus on open, close on Escape and return focus to the control that opened them. Acting inside an open dialog (changing a ticket's status, say) must leave focus on the control being used, not pull it back to the dialog.
