      ...
      try {
        ...
        await prisma.projectFile.create({
          data: {
            ...
            type: file.mimetype,
            size: file.size,
            // TT-092: this was the string 'system-proposal-generator', which is not a
            // member id — it is what stopped uploadedBy taking a foreign key. Null is
            // the representation for "generated by the system" now.
            uploadedBy: null,
          }
        });
  ...
  try {
    ...
    await prisma.projectFile.create({
      data: {
        ...
        size: req.file.size,
        // TT-092: the sibling site above was changed to null, but this one kept its
        // sentinel string. uploadedBy is a foreign key now, so 'system-section-generator'
        // is rejected and this endpoint fails — the column being `String?` means the
        // compiler cannot see it. Null is the representation for "generated by the
        // system"; both system writers now use it.
        uploadedBy: null,
      }
    });
